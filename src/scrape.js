...
async function getProposals() {
  try {
    ...
    const response = await axios.get(
      "https://hub.snapshot.page/api/balancer/proposals"
    );
    ...
    for (const proposal in data) {
      ...
      let duplicateProposal = proposals.includes(proposal);
      ...
      let duplicateRelayerIpfsHash = relayers.includes(relayerIpfsHash);
      if (duplicateRelayerIpfsHash) {
        console.log(`Duplicate relayer hash: ${relayerIpfsHash}`);
      } else {
        fs.appendFileSync(
          "./data/proposals-relayers.txt",
          relayerIpfsHash + "\n"
        );
        console.log(
...
async function getVotes() {
  try {
    let proposals = fs.readFileSync("./data/proposals.txt", "utf8").split("\n");
    ...
    for (const proposal of proposals) {
      ...
      for (const vote in data) {
        ...
        let duplicateVote = votes.includes(authorIpfsHash);
        if (duplicateVote) {
        ...
        if (duplicateRelayerIpfsHash) {
          ...
          fs.appendFileSync(
            "./data/votes-relayers.txt",
            relayerIpfsHash + "\n"
          );
          console.log(
            `Added relayer hash    : ${relayerIpfsHash} to votes-relayers.txt`
          );
        }
      }
    }
